...
> Aquest fitxer serveix a la vegada com a **guia operativa per a agents d'IA**
> i com a **document de projecte**. Llegeix-lo sencer abans de tocar res.

---

## 1. Què és això

**Nau Bostik** és un espai cultural autogestionat del barri de la Sagrera
...
Dades físiques reals (corregir arreu del repo abans de producció):

```
Nau Bostik — Ferran Turné 1-11, 08027 Barcelona (la Sagrera)
```
...
|---|---|---|---|
| Local | Desenvolupament amb `hugo server` | `http://localhost:1313/` | Privat |
| Staging | Vista prèvia per a l'equip | `https://naubostik.netlify.app/` | Privat (Basic Auth via Edge Function) |
| Producció | `naubostik.com` | `https://naubostik.com/` | Públic quan estigui aprovat |

Regles:
- La `baseURL` **no** va hardcoded a `hugo.toml`; es passa per flag.
- GitHub Pages **no** pot fer autenticació: no serveix com a staging privat.
- La documentació interna (aquest fitxer, `HISTORIA.md`) **no** ha d'anar
  mai a un repo públic.
...
- **CSS:** vanilla, `themes/thema/static/css/main.css`. Patró: **variables
  CSS + classes curtes** (`.espai-card`, `.planta-grid`). Sense
  preprocessadors ni Tailwind.
  - `--color-primary: #1a1a1a`, `--color-accent: #c41e3a` (vermell Nau),
...
- **CMS:** Decap CMS v3 (`/admin/`, git-gateway + Netlify Identity).
  **Estat: reavaluar.** Candidats: Sveltia CMS (drop-in), TinaCMS (visual
  editing), headless propi. Fins a decisió: no ampliar Decap ni trencar
  `static/admin/config.yml`.
- **Hosting:** Netlify (`netlify.toml`). GitHub Pages descartat com a
  staging (no suporta auth).
...
hugo.toml           # config (baseURL via flag, no hardcoded)
